...
export class ThemeStore {
  ...
  async initSessionThemeSync(sessionId: number) {
    console.log('🔗 initSessionThemeSync called for session:', sessionId);
    try {
      await this.hub.connect();
      console.log('✅ SignalR connected');
  ...
  applyThemeToSession(sessionId: number, theme: ThemeModel, hostId: string) {
    console.log('🎨 applyThemeToSession called with sessionId:', sessionId, 'theme:', theme);
    ...
    this.api.applyThemeToSession({ sessionId, themeId: theme.id }, hostId).subscribe({
      next: () => {
        console.log('✅ Theme saved to session:', sessionId);
  ...
  async disconnectSessionThemeSync(sessionId: number) {
    console.log('🔗 disconnectSessionThemeSync called for session:', sessionId);
    try {
